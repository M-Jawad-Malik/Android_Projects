...
class ChatLogActivity : AppCompatActivity() {
    ...
    var touser:MessengerUserStruct?=null
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_chat_log)
        touser=  intent.getParcelableExtra<MessengerUserStruct>(NewMessageActivity.USER_KEY)
            ...
            if(resultCode== Activity.RESULT_OK)
            {
                try{
                    if(data!!.data!=null)
                    {
                        ...
                        val user=  intent.getParcelableExtra<MessengerUserStruct>(NewMessageActivity.USER_KEY)
...
class ChatFromItem(val text: String, val user: MessengerUserStruct):Item<GroupieViewHolder>(){
   ...
   Picasso.get().load(user.profileImageUrl).into(viewHolder.itemView.imagechatfromrow)
            ...
            viewHolder.itemView.imagechatfromrow.setImageResource(R.drawable.avatar)
    }
...
class ChatToItem(val text: String, val user: MessengerUserStruct):Item<GroupieViewHolder>(){
    override fun bind(viewHolder: GroupieViewHolder, position: Int) {
        if(text.indexOf("image/")!=-1){
        ...
        if(user.profileImageUrl!="") {
            ...
            viewHolder.itemView.imagechattorow.setImageResource(R.drawable.avatar)
        }
    }
