...
public class AddTourActivity extends AppCompatActivity {
    ...
    TextInputEditText titleAR, titleEN,titleUR;
...
try {
    ...
    public void prepareView()
    {
        ...
        titleUR=findViewById(R.id.titleU);
    ...
    public void addTour(View v) {
        if (AdminPlaceAdapter.chosen_places.size() >= 3) {
            ...
            if (Text_checker()) {
                ...
                mTour = new Tour(makeCategoryArabic(chosen_place), makeCategoryEnglish(chosen_place),makeCategoryUrdu(chosen_place), comments, 0, images(chosen_place), 0, chosen_place, 0.0, titleAR.getText().toString().trim(), titleEN.getText().toString().trim(), titleUR.getText().toString().trim(), newTourRef.getId(), makeKeywords(chosen_place));
    ...
    private ArrayList<String> makeCategoryUrdu(List<Place> mPlaces) {
        ArrayList<String> ur = new ArrayList<>();
        for (int i = 0; i < mPlaces.size(); i++) {
            if (!ur.contains(mPlaces.get(i).categoryUR))
                ur.add(mPlaces.get(i).categoryUR);
        }
        return ur;
    }
